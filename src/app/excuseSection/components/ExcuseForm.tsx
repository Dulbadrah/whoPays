...
//   const [playerName, setPlayerName] = useState<string>(""); // Тоглогчийн нэрийг хадгалах state
  ...
  const API_URL = "http://localhost:4200/roast"; // Таны backend-ийн зам (PORT 4200 дээр ажиллаж байвал)
  // const API_KEY = "YOUR_API_KEY_HERE" // API түлхүүр frontend-д хэрэггүй
...
//   };
  ...
  const handleSubmit = async (e: FormEvent<HTMLFormElement>) => {
    ...
    setLoading(true);
    setError("");
    setRoastResult(null); // Өмнөх хариуг цэвэрлэх
    ...
    try {
      ...
      const submissionId = `client-${Date.now()}-${Math.random().toString(36).substring(2, 9)}`;
      const playersData = [
        {
      ...
      const response = await fetch(API_URL, {
        ...
        headers: {
          ...
          // Authorization: `Bearer ${API_KEY}`, // Хэрэв backend-д шаардлагатай бол нэмнэ.
        },
        body: JSON.stringify({ reasons: playersData }), // players массивыг илгээх
      });
      ...
      const data: RoastResult = await response.json(); // Backend-ээс ирэх хариуг хүлээн авах
      setRoastResult(data);
      setShowRoastModal(true); // Модалыг харуулах
      setReason(""); // Шалтгааныг цэвэрлэх (нэрийг үлдээж болно)
    } catch (err: any) {
      setError(err.message || "Алдаа гарлаа. Дахин илгээнэ үү");
      console.error("Frontend Roast error:", err);
    } finally {
      setLoading(false);
    }
  };
        ...
        {showRoastModal && roastResult && (
          <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center p-4 z-50">
            <div className="bg-white rounded-2xl p-6 sm:p-8 shadow-2xl max-w-sm w-full text-center transform scale-100 animate-fade-in">
              <h3 className="text-2xl font-bold text-green-700 mb-4">💰 Төлбөр төлөгч сонгогдлоо! 💰</h3>
              <p className="text-xl text-gray-800 font-semibold mb-3">
                🎉 <span className="text-blue-600">{roastResult.chosenPlayerName}</span> 🎉 өнөөдөр төлбөрөө төлнө!
              </p>
